...
class Data(Dataset):
    def __init__(self, cfg):
        ...
        self.samples = []
        files = os.listdir(self.cfg.datapath+'DUTS-TE-Image/')
        for file in files:
    ...
    def __getitem__(self, idx):
        ...
        image = cv2.imread(self.cfg.datapath+'/DUTS-TE-Image/'+name+'.jpg')[:,:,::-1].astype(np.float32) 
        mask  = cv2.imread(self.cfg.datapath+'/DUTS-TE-Mask/' +name+'.png', 0).astype(np.float32)
        shape = mask.shape
